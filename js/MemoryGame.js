...
class MemoryGame {
	...
	buildGrid () {
		...
		this.selector.innerHTML = htmlstr;
	...
	checkMatch () {
		if(this.revealed.length === 2 && this.revealed[0].getAttribute('data-class') === this.revealed[1].getAttribute('data-class')){
			this.score+=1
			if(this.score==8){
				setTimeout(()=>{
					window.location.href='bingdundun.html'
				},3000)
